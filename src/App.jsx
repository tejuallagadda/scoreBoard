...
export default function ScoreTracker() {
  ...
  return (
    <div className="min-h-screen p-4 bg-gray-100 flex items-center justify-center">
      <div className="w-full max-w-screen-lg bg-white p-4 sm:p-6 rounded-lg shadow overflow-hidden">
        <h1 className="text-2xl sm:text-3xl font-bold text-center text-blue-700 mb-4">🏆 Score Tracker</h1>
            ...
            players.map((player, index) => (
              <div key={index} className="border border-gray-200 rounded-lg p-3 bg-gray-50 hover:bg-gray-100 transition">
                <div className="flex justify-between items-center mb-2">
                  <span className="font-semibold text-lg truncate" title={player.name}>{player.name}</span>
                  <span className="text-blue-600 font-bold">{player.total}</span>
                </div>
                <input
                  className="border border-gray-300 rounded px-3 py-1 w-full focus:outline-none focus:ring-2 focus:ring-blue-300"
                  type="number"
